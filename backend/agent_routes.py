...
def get_db_and_models():
    ...
    if not hasattr(g, '_database'):
        ...
        from app import User, Quotation
        g._database = db
        g._user_model = User
        g._quotation_model = Quotation
    
    return g._database, g._user_model, g._quotation_model
...
def create_agent_registration():
    ...
    try:
        ...
        # Get database and models from current app context
        db, User, Quotation = get_db_and_models()
...
def update_agent_services(quotation_id):
    ...
    try:
        ...
        for service in services:
            service_data = {
                'id': service.get('id', service['name'].replace(' ', '_').lower()),
                'label': service['name'],
